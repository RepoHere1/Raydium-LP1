...
- Loopback web UI: run `.\scripts\run_dashboard_web.ps1` then open `http://127.0.0.1:8844/` — `.\scripts\run_scan.ps1 -Loop` passes `--dashboard --reload-config-each-scan` so HTML **Save** updates apply on the next scan cycle without restarting Python
